...
function storeStatusFromHours(hours){
  const schedule=hours||defaultBusinessHours
  const now=mexicoNowParts()
  const today=schedule[now.day]||defaultBusinessHours[now.day]
  if(!today||today.closed)return {open:false,day:now.day,time:now.time,today}
  return {open:now.time>=today.open&&now.time<today.close,day:now.day,time:now.time,today}
}
...
function useCatalog(){
  ...
  const [settings,setSettings]=useState({minimum_order:200,points_reward_cost:250,points_reward_product_id:null,business_hours:defaultBusinessHours})
  const [loading,setLoading]=useState(supabaseConfigured)
  const refresh=async()=>{
    if(!supabase){setLoading(false);return}
    setLoading(true)
...
function App(){
  ...
  const storeStatus=useMemo(()=>storeStatusFromHours(catalog.settings?.business_hours),[catalog.settings?.business_hours,storeClockTick])
  ...
  const addConfigured=item=>{
    if(!storeStatusFromHours(catalog.settings?.business_hours).open){setConfiguring(null);alert('KYO acaba de cerrar. No pudimos agregar este producto al carrito.');return}
...
function ProductCard({p,add,branchId,storeOpen=true}){const branchUnavailable=branchId&&p.branchAvailability?.[branchId]===false;const storeClosed=!storeOpen;return <article className={`product-card ${branchUnavailable||storeClosed?'branch-unavailable':''}`}><div className="product-img"><img src={p.image||p.image_url} alt={p.name}/>{p.spicy&&<span className="spicy"><Flame size={13}/> Spicy</span>}{(branchUnavailable||storeClosed)&&<span className="branch-soldout">{storeClosed?'Cerrado ahora':'No disponible aquí'}</span>}</div><div className="product-body"><small>{p.category}</small><h3>{p.name}</h3><p>{p.desc||p.description}</p><div><strong>{productDisplayPrice(p)}</strong><button className="add-btn" disabled={branchUnavailable||storeClosed} onClick={()=>!branchUnavailable&&!storeClosed&&add(p)} aria-label={`Agregar ${p.name}`}><Plus/></button></div></div></article>}
