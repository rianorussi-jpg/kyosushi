...
import { supabase, supabaseConfigured, MENU_BUCKET } from './supabase'
...
function AdminGate({auth,children}){
...
function AdminLogin(){const [email,setEmail]=useState('');const [password,setPassword]=useState('');const [error,setError]=useState('');const [busy,setBusy]=useState(false);const submit=async e=>{e.preventDefault();if(!supabase)return setError('Configura VITE_SUPABASE_URL y VITE_SUPABASE_ANON_KEY.');setBusy(true);const {error}=await supabase.auth.signInWithPassword({email,password});setBusy(false);if(error)setError(error.message)};return <main className="admin-login"><div className="admin-login-card"><Brand/><span className="admin-pill">PANEL ADMINISTRATIVO</span><h1>Control de KYO</h1><p>Ingresa con el correo y contraseña de administración.</p><form onSubmit={submit}><label>Correo<input type="email" required value={email} onChange={e=>setEmail(e.target.value)}/></label><label>Contraseña<input type="password" required value={password} onChange={e=>setPassword(e.target.value)}/></label>{error&&<div className="form-message">{error}</div>}<button className="primary full" disabled={busy}>{busy?'Entrando...':'Entrar al panel'}</button></form></div></main>}

function AdminPanel({auth,catalog}){
  ...
  useEffect(()=>{loadOrders(); const ch=supabase?.channel('admin-orders').on('postgres_changes',{event:'*',schema:'public',table:'orders'},loadOrders).subscribe(); return ()=>{if(ch)supabase.removeChannel(ch)}},[])
